...
Source folder: `G:\ac-grpo\outputs\2026-04-22_231909_all_experiments\baseline_dense_reward_noise`
...
- test_clean (CLEAN, sigma=0.10): best mean return is `BASELINE_DENSE` in `REWARD_NOISE` with 277.6 ± 20.8 and success 1.000 ± 0.000.
- test_obs_noise (OBS_NOISE, sigma=0.10): best mean return is `BASELINE_DENSE` in `REWARD_NOISE` with 215.6 ± 108.3 and success 0.725 ± 0.447.
- test_obs_noise_hard (OBS_NOISE, sigma=0.20): best mean return is `BASELINE_DENSE` in `REWARD_NOISE` with 24.6 ± 35.0 and success 0.000 ± 0.000.
