...
Source folder: `G:\ac-grpo\outputs\2026-04-23_235329_all_experiments\ac_lite_dense_clean`
...
| AC-LITE (Dense) | 212.1 ± 110.7 / 0.710 ± 0.454 | n/a | n/a |
...
|---|---|---:|---:|
| CLEAN | AC_LITE_DENSE | 0.275 | 0.293 |

## Result analysis

The final training-window summary shows the following strongest method per mode:

- CLEAN: `AC_LITE_DENSE` has the highest mean final return (254.6) with mean final success 0.890.
...
- test_clean (CLEAN, sigma=0.10): best mean return is `AC_LITE_DENSE` in `CLEAN` with 277.4 ± 20.6 and success 1.000 ± 0.000.
- test_obs_noise (OBS_NOISE, sigma=0.10): best mean return is `AC_LITE_DENSE` in `CLEAN` with 212.1 ± 110.7 and success 0.710 ± 0.454.
- test_obs_noise_hard (OBS_NOISE, sigma=0.20): best mean return is `AC_LITE_DENSE` in `CLEAN` with 21.7 ± 30.8 and success 0.000 ± 0.000.
